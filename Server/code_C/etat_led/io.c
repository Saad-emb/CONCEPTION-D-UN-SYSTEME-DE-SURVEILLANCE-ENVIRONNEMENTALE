...
int load_gpio_line(S_GPIO_LINE *ps_line, char id_number[4], int i_direction)
{
    ...
    rewind(p_gpio_line);//pointe debut file
    strcpy(ps_line->id_number, id_number);//destination source
    fwrite(&ps_line->id_number, sizeof(char), 2, p_gpio_line); //wite value that is pointed  size  The sizeof(char) argument specifies the size of each element to be written (which is 1 byte for a character), and the 2 argument specifies the number of elements to be written.
    fclose(p_gpio_line);
...
int set_gpio_direction(S_GPIO_LINE *ps_line, int i_direction)
{
    ...
    sprintf(gpio_direction, "/sys/class/gpio/gpio%s/direction", ps_line->id_number);//https://www.youtube.com/watch?v=Ps-KajI0asw&ab_channel=PortfolioCourses
//stocke dans gpio_direction  
//sprintf pour specifier le numero de gpio mamknnch ndiro f open  
    ...
    if( i_direction ) {
        fputc((int)'i', p_gpio_direction);//code ascii??
...
int get_gpio_line(S_GPIO_LINE *ps_line)
{
    ...
    if( ps_line->direction == 1 ) {
        ...
        value = fgetc (p_gpio_value) - 48;//si 0 48-48 si 1 49-48 ascii to int 
